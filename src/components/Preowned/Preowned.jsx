...
const Preowned = () => {
  ...
  const filtersChosen =
    query?.productName?.length +
    query?.condition?.length +
    query?.storage?.length +
    query?.color?.length +
    (query?.price[0] > 10 || query?.price[1] < 1480 ? 1 : 0);
  ...
  const requestProduct = () => {
    setLoading(true);
    axiosInstance
      .post(`products/${prodcutsReq}/${productSkip.current}`, query)
      .then((res) => {
        if (res.data.length) {
  ...
  useEffect(() => {
    if (!location.search) {
      ...
      setLoading(true);
      axiosInstance
        .get(`searchproducts?search=${location.search}`)
  ...
  return (
    <main>
          ...
          {loading ? (
            ...
            <>
              ...
              <button
                ...
                disabled={moreProductsInDb ? false : true}
              >
                {" "}
                See More
              </button>
      ...
      <div className="mobile-filter">
        <button id="filter-btn" onClick={handleFiltershowHide}>
          <span>Filter</span>
