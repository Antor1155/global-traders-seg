...
const AdminHome = () => {
  ...
  const handleUpdateMonthlySells = async (e) => {
    e.preventDefault();
    const amount = e.target.amount.value;

    if (!isNaN(amount)) {
      axiosInstance
        .post("this-month-sold-items", { name: "gt-sells", amount })
        .then((res) => {
  ...
  return (
    <section id="adminHome">
      ...
      <div style={{ margin: "5rem auto", fontWeight: "bold" }}>
        <p>Update current month's sell amount to show on website</p>
        <form
          style={{ display: "flex", gap: "1rem" }}
          onSubmit={handleUpdateMonthlySells}
        >
